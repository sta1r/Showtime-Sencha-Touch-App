...
function getFileList(projectFile) {
    ...
    if (phantom.fs.pathExists(projectFile)) {
        try {
            content = phantom.fs.readFile(projectFile);
...
function extractLocaleContent(file, projectRoot) {
    var lines = phantom.fs.readFile(projectRoot + file.path + file.name).split('\n'),
        chunks = [],
        buffer = [],
        meta = null,
        fileInfo = {},
...
phantom.fs.writeFile(args.target, buffer.join('\n'));
